...
ns = ns.dropna()
    ...
    # cross validation and apply RF machine learning techniques
    ...
    stratified_9folds = cv.split(wdata, wlabel)
...
model = RandomForestClassifier(n_estimators = 60, max_depth = 2, random_state = 42).fit(tr,trl)
# evaluate model
scores = cross_val_score(model, wdata, wlabel, scoring='accuracy', cv=cv, n_jobs=-1)
# report performance
print('Accuracy: %.3f (%.3f)' % (mean(scores), std(scores)))

## the results gives 0.739 of accuracy with std of 0.037 for 9 fold cross-validation.
## here is to optimize the best kfold for LG model.
